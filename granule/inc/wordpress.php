...
function granule_enqueue() {
	...
	if ( granule_has_featured_posts() ) {
		wp_enqueue_script( 'granule-script-slider', get_theme_file_uri( '/assets/scripts/jquery.slider.js' ), array( 'jquery' ), '1.5.1', false );
	}
 ...
 */
function granule_resource_hints( $urls, $relation_type ) {

	if ( wp_style_is( 'granule-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
...
add_filter( 'wp_resource_hints', 'granule_resource_hints', 10, 2 );
